...
// The "function selector" is the first 4 bytes of the function signature.  eg. 0xa9059cbb
// The "function signature" is a string that defines the function name & parameters. eg. "transfer(address,uint256)"
// Let's look at this
...
contract CallAnything {
    ...
    function transfer(address someAddress, uint256 amt) public {
        s_someAddress = someAddress;
        s_amount = amt;
    }

    function getSelectorOne() public pure returns (bytes4 selector) {
        selector = bytes4(keccak256(bytes("transfer(address,uint256)"))); // no space in between parameters

        // CHECK selector value directly(closest to Remix UX) ==> forge create + cast 
        // we can deploy directly using `forge create` then call function using `cast`
        // forge create src/sublesson/CallAnything.sol:CallAnything --rpc-url http://127.0.0.1:8545 --private-key ... --broadcast
        // cast call <CONTRACT_ADDRESS> "getSelectorOne()(bytes4)"
    }
